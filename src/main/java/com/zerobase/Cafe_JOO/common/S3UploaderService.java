package com.zerobase.Cafe_JOO.common;
...
public class S3UploaderService {
    ...
    // @Valid("${cloud.aws.s3.bucket}") 어노테이션을 사용할려면 생성자를 따로 만들어야 한다.
    @Autowired
    ...
    // multipartFile = 업로드할 파ㄹ일, dirName = 업로드할 디렉토리 이름
    // 이미지 파일 업로드
    public String uploadFileToS3(MultipartFile multipartFile, String dirName) throws IOException {
        ...
        try (InputStream inputStream = multipartFile.getInputStream()) {
            // 파일을 AmazonS3 버킷에 업로드, with~ 접근 허가 요청
